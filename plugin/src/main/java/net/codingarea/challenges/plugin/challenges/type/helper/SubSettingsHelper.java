			...
			for (EntityType type : EntityType.values()) {
				...
				try {
					...
					builder.addSetting(type.name(), new ItemBuilder(spawnEgg,
							DefaultItem.getItemPrefix() + BukkitStringUtils.getEntityName(type)).build());
				} catch (Exception ex) {
					builder.addSetting(type.name(), new ItemBuilder(Material.STRUCTURE_VOID,
							DefaultItem.getItemPrefix() + BukkitStringUtils.getEntityName((type))));
				}
			}
		});
	}
		...
		return SubSettingsBuilder.createChooseMultipleItem(BLOCK).fill(builder -> {
			...
			for (Material material : Material.values()) {
				if (material.isBlock() && material.isItem() && !BukkitReflectionUtils.isAir(material)) {
					builder.addSetting(material.name(), new ItemBuilder(material, DefaultItem.getItemPrefix() + BukkitStringUtils.getItemName(material)).build());
				}
			}
		});
	}
