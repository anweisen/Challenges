 ...
 */
@Since("2.0")
public class HitMobIsPreviousChallenge extends Setting {

	public HitMobIsPreviousChallenge() {
	...
	protected void onEnable() {
		bossbar.setContent((bossbar, player) -> {
			...
			EntityType type = getType(player, null);
	...
	public void onEntityDamageByEntity(@Nonnull EntityDamageByEntityEvent event) {
		...
		EntityType type = getType(player, event.getEntityType());
	...
	private EntityType getType(@Nonnull Player player, EntityType defaultType) {
