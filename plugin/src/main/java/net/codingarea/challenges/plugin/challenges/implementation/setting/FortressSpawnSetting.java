 ...
 */
@Since("2.0")
public class FortressSpawnSetting extends Setting {
	...
	public void onNetherPortal(@Nonnull PlayerTeleportEvent event) {
		...
		if (event.getTo().getWorld().getEnvironment() == Environment.NETHER) {
			...
			if (location == null) {
				Message.forName("unable-to-find-fortress").send(event.getPlayer(), Prefix.CHALLENGES);
				return;
			}
	...
	private Location getNetherPortal(@Nonnull Location overworldPortal, @Nonnull World nether) {
		...
		// Find next fortress
		Location fortress = nether.locateNearestStructure(new Location(nether, 0, 100, 0), StructureType.NETHER_FORTRESS, 10000, false);
