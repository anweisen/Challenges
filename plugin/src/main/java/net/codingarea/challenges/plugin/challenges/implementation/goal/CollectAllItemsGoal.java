...
public class CollectAllItemsGoal extends SettingGoal implements SenderCommand {
	...
	protected void onEnable() {
		bossbar.setContent((bossbar, player) -> {
			...
			bossbar.setTitle(Message.forName("bossbar-all-items-current").asString(StringUtils.getEnumName(currentItem), totalItemsCount - itemsToFind.size() + 1, totalItemsCount));
		});
		bossbar.show();
	}
